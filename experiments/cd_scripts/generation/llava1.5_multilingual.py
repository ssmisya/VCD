...
def main():
    # language_list1=["es","de","fr","it","pt","ru","ja","ko","zh","ar"]
    languages = "en ar  bg  bn  de  el  es  fa  fr  gu  hi  id  it  ja  jv  ko  ml  mr  ms  my  nl  pt  ru  sv  sw  ta  te  th  tr  uk  ur  vi  zh"
    language_list = languages.split()
    # language_list = [item for item in language_list2 if item not in language_list1]
    dataset_list=["coco"]
        ...
        for j in range(len(type_list)):
            for k in range(len(language_list)):
                ...
                if os.path.exists(output_file):
                    ...
                    #         + " ".join(cmd_args)
                    #         + f" 1>{log_path} 2>&1")
                    run_command(f"nohup srun -p {partition} -n1 -N1 --gres=gpu:1 --quotatype=auto -c 16 --job-name=generate "
                            + f"--output={log_path} "
                            + f"--error={log_path} "
                            + "python /mnt/petrelfs/songmingyang/code/VCD/experiments/eval/object_hallucination_vqa_llava.py "
                            + " ".join(cmd_args)
